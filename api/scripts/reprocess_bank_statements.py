...
def reprocess_bank_statement(tenant_id: int, statement_id: int):
    """Reprocess a specific bank statement."""
    try:
        ...
        print(f"Error reprocessing bank statement: {e}")
        import traceback
        traceback.print_exc()

def list_bank_statements(tenant_id: int):
    ...
    try:
        ...
        print(f"Error listing bank statements: {e}")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python reprocess_bank_statements.py list <tenant_id>")
        print("  python reprocess_bank_statements.py reprocess <tenant_id> <statement_id>")
        sys.exit(1)
    ...
    if command == "list":
        if len(sys.argv) != 3:
            print("Usage: python reprocess_bank_statements.py list <tenant_id>")
            sys.exit(1)
        tenant_id = int(sys.argv[2])
        list_bank_statements(tenant_id)
        
    elif command == "reprocess":
        if len(sys.argv) != 4:
            print("Usage: python reprocess_bank_statements.py reprocess <tenant_id> <statement_id>")
            sys.exit(1)
        tenant_id = int(sys.argv[2])
        statement_id = int(sys.argv[3])
        reprocess_bank_statement(tenant_id, statement_id)
