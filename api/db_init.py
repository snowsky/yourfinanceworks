...
def create_gamification_tables(database_url):
    """Create gamification tables using SQLAlchemy models."""
    try:
        from core.models.gamification import Base as GamificationBase

        engine = create_engine(database_url)

        # Create all gamification tables from models
        GamificationBase.metadata.create_all(bind=engine)

        logger.info("Gamification tables created successfully using SQLAlchemy models")
        return True
