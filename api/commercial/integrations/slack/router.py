...
from core.models.models_per_tenant import Client, Invoice, Payment, User
from core.schemas.client import ClientCreate
from core.schemas.invoice import InvoiceCreate
...
class SlackInvoiceBot:
    ...
    async def _list_clients(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            for client in clients:
                # Calculate outstanding balance in Python
                pending_invoices = db.query(Invoice).filter(
                    Invoice.client_id == client.id,
                    Invoice.status.in_(['pending', 'sent'])
                ).all()

                outstanding = sum(float(invoice.amount) if invoice.amount else 0.0
    ...
    async def _get_recommendations(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            total_outstanding = db.query(func.sum(func.cast(Invoice.amount, Float))).filter(
                Invoice.status.in_(['pending', 'sent'])
            ).scalar() or 0
    ...
    async def _list_reminders(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            from core.models.models_per_tenant import Reminder
