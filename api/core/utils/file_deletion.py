...
async def delete_file_from_storage(
    ...
    try:
        ...
        # Skip local deletion for cloud storage paths (those that don't start with 'attachments/')
        # Cloud paths like 'api/batch_files/tenant_1/...' should not be validated as local paths
        if file_path.startswith('attachments/') or file_path.startswith('/'):
            ...
            logger.debug(f"Skipping local deletion for cloud storage path: {file_path}")
