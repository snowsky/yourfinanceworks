...
def upgrade():
    ...
    op.execute("ALTER TABLE expenses ADD COLUMN IF NOT EXISTS is_audited BOOLEAN NOT NULL DEFAULT FALSE")
    op.execute("ALTER TABLE expenses ADD COLUMN IF NOT EXISTS last_audited_at TIMESTAMP WITH TIME ZONE")
    
    # Add is_audited and last_audited_at to invoices table
    op.execute("ALTER TABLE invoices ADD COLUMN IF NOT EXISTS is_audited BOOLEAN NOT NULL DEFAULT FALSE")
    op.execute("ALTER TABLE invoices ADD COLUMN IF NOT EXISTS last_audited_at TIMESTAMP WITH TIME ZONE")
    
    # Add is_audited and last_audited_at to bank_statement_transactions table
    op.execute("ALTER TABLE bank_statement_transactions ADD COLUMN IF NOT EXISTS is_audited BOOLEAN NOT NULL DEFAULT FALSE")
    op.execute("ALTER TABLE bank_statement_transactions ADD COLUMN IF NOT EXISTS last_audited_at TIMESTAMP WITH TIME ZONE")
