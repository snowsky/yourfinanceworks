...
export function LinkTransferModal({
  ...
  React.useEffect(() => {
    ...
    bankStatementApi
      ...
      .catch(() => toast.error(t('statements.link_transfer.load_statements_failed')))
  ...
  const handleSelectStatement = async (stmt: BankStatementSummary) => {
    ...
    try {
      ...
      toast.error(t('statements.link_transfer.load_transactions_failed'));
      setStep('pick-statement');
    } finally {
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      if (linkForSource) {
        ...
        toast.success(t('statements.link_transfer.link_success'));
      } else {
      ...
      toast.error(e?.message || t('statements.link_transfer.link_failed'));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            {t('statements.link_transfer.description')}
          </DialogDescription>
        </DialogHeader>

        {/* Step indicator */}
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className={step === 'pick-statement' ? 'text-primary font-medium' : ''}>{t('statements.link_transfer.step1')}</span>
          <ArrowRight className="w-3 h-3" />
          <span className={step === 'pick-transaction' ? 'text-primary font-medium' : ''}>{t('statements.link_transfer.step2')}</span>
          <ArrowRight className="w-3 h-3" />
          <span className={step === 'confirm' ? 'text-primary font-medium' : ''}>{t('statements.link_transfer.step3')}</span>
        </div>
        ...
        <div className="flex-1 overflow-hidden">
                ...
                <ScrollArea className="h-72">
                  <div className="space-y-1 pr-2">
                    ...
                    {!loadingStatements && statements.filter((s) =>
                      ...
                      <p className="text-center text-sm text-muted-foreground py-8">{t('statements.link_transfer.no_other_statements')}</p>
                    )}
                  </div>
                </ScrollArea>
              )}
          ...
          {step === 'pick-transaction' && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <Label className="text-xs text-muted-foreground shrink-0">{t('statements.link_transfer.link_type')}</Label>
                <Select value={linkType} onValueChange={(v) => setLinkType(v as typeof linkType)}>
                  <SelectTrigger className="h-7 w-44 text-xs">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="transfer">{t('statements.link_transfer.type_transfer')}</SelectItem>
                    <SelectItem value="fx_conversion">{t('statements.link_transfer.type_fx_conversion')}</SelectItem>
                  </SelectContent>
                </Select>
                <span className="text-xs text-muted-foreground">
                  {linkType === 'transfer' ? t('statements.link_transfer.showing_opposite_type') : t('statements.link_transfer.showing_all_transactions')}
                </span>
              </div>
              {loadingDetail ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b text-muted-foreground">
                        ...
                        <th className="text-left py-1 pr-2">{t('common.description')}</th>
                        <th className="text-right py-1 pr-2">{t('common.amount')}</th>
                      ...
                      {filteredTransactions.length === 0 && (
                        <tr>
                          <td colSpan={4} className="text-center text-muted-foreground py-8">
                            {t('statements.link_transfer.no_matching_transactions')}
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          {step === 'confirm' && selectedTransaction && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  <Label className="text-sm shrink-0">{t('statements.link_transfer.link_type')}</Label>
                  <Select value={linkType} onValueChange={(v) => setLinkType(v as typeof linkType)}>
                ...
                <div className="space-y-1">
                  <Label className="text-sm">{t('statements.link_transfer.notes_optional')}</Label>
                  <Textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder={t('statements.link_transfer.notes_placeholder')}
        ...
        <DialogFooter>
          ...
          {step === 'confirm' && (
            <Button onClick={handleLink} disabled={submitting} className="gap-2">
              ...
              {t('statements.link_transfer.button_link')}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
